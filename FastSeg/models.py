...
class NNConv5_Decoder(nn.Module):
  def __init__(self, kernel_size, depthwise=True):
    ...
    if (depthwise):
      ...
      self.conv5 = nn.Sequential(DWConvBlock(64,64,kernel_size,1),ConvBlock(64,64,1,1,0))
    else:
...
class FastSeg(nn.Module):
  def __init__(self,kernel_size=5):
    super(FastSeg,self).__init__()
    self.encoder= MobileNet_Encoder()
    self.decoder = NNConv5_Decoder(kernel_size)
    self.output = ConvBlock(64,40,1,1,0)
  def forward(self,x):
    ...
    x= F.interpolate(self.decoder.conv2(x) , scale_factor=2, mode='nearest') + layer3
...
class FastSegV2(nn.Module):
  def __init__(self,kernel_size=5):
    super(FastSegV2,self).__init__()
    self.encoder= MobileNetV2_Encoder()
    self.decoder = NNConv5_DecoderV2(kernel_size)
    self.output = ConvBlock(32,40,1,1,0)
  def forward(self,x):
